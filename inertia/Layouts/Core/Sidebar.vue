  ...
  >
    <div class="h-full px-3 py-4 overflow-y-auto flex flex-col">
      ...
      <div class="flex items-center justify-between mb-4 px-2">
        <h1 class="text-lg font-bold text-gray-900 dark:text-white">MCHLNGL.DV</h1>
      </div>
      <!-- Nav -->
      <ul class="space-y-2 font-medium">
        <li v-for="item in menu" :key="item.label">
          <Link
            ...
            :class="{ 'bg-gray-100 dark:bg-gray-800 font-semibold': page.component === item.label }"
          >
            <component
              :is="item.icon"
            ...
            />
            <span
              class="ml-3 capitalize text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white"
            >
              {{ item.label }}
            </span>
          </Link>
...
// Assuming you have named routes matching these labels in your AdonisJS backend
const menu = ref([
